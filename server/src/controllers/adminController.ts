 ...
 * Flips is_active on an announcement. Used by the Eye/EyeOff toggle in AnnouncementsTab.
 */
export const toggleAnnouncement = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const result = await db.query(
      'UPDATE announcements SET is_active = NOT is_active WHERE id = $1 RETURNING is_active',
      [id]
    );
